...
async function uploadImage(file, bucket = 'product-images') {
  ...
  // Используем FormData - это стандартный и надежный способ для Supabase Storage
  const formData = new FormData();
  formData.append('file', file);

  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/${bucket}/${randomName}`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_KEY,
      Authorization: `Bearer ${SUPABASE_KEY}`
    },
    body: formData // Без указания Content-Type, FormData сам это сделает!
  });

  if (response.ok) {
    return `${SUPABASE_URL}/storage/v1/object/public/${bucket}/${randomName}`;
  } else {
    // Выводим настоящую ошибку от сервера для отладки
    const errorData = await response.text();
    console.error('Ошибка Supabase:', errorData);
    alert(`⚠️ Ошибка загрузки фото! Проверь интернет и бакет '${bucket}'. Детали: ${errorData}`);
    return null;
  }
}
