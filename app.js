...
async function initProfile() {
  try {
    ...
    // Если админ — кнопка остаётся видимой
  } catch (e) {
...
document.getElementById('save-product-btn').onclick = async () => {
  ...
  if (!title.trim()) {
    alert("⚠️ Ты забыл написать название товара! Впиши его в поле «Название».");
    return;
  }
  if (!price || parseFloat(price) <= 0) {
    alert("⚠️ Ты забыл указать цену! Впиши её в поле «Цена (₽)».");
    return;
  }
  if (imageFiles.length === 0) {
    alert("⚠️ Ты не загрузил ни одного фото! Нажми на кнопку «📸 Нажми, чтобы выбрать фото» и выбери картинку.");
    return;
  }
  if (!subId) {
    alert("⚠️ Ты не выбрал тип одежды! Нажми на список «Выберите тип...» и выбери нужный.");
    return;
  }
  if (!supId) {
    alert("⚠️ Ты не выбрал поставщика! Нажми на список «Выберите поставщика...» и выбери, от кого товар.");
    return;
  }
  ...
  if (response.ok) {
    ...
    alert("❌ Ошибка! Не получилось сохранить товар. Попробуй ещё раз.");
  }
};
...
document.getElementById('save-supplier-btn').onclick = async () => {
  ...
  if (!name.trim()) {
    alert("⚠️ Ты забыл написать название поставщика! Впиши его в поле «Название».");
    return;
  }
  if (!logoFile) {
    alert("⚠️ Ты не загрузил логотип! Нажми на «📸 Загрузить логотип» и выбери картинку.");
    return;
  }
...
document.getElementById('modal-save-btn').onclick = async () => {
  ...
  if (!name.trim()) {
    alert("⚠️ Ты забыл ввести название! Впиши его в поле «Название».");
    return;
  }
  if (!imgFile) {
    alert("⚠️ Ты не загрузил иконку! Нажми на «📸 Загрузить иконку» и выбери картинку.");
    return;
  }
